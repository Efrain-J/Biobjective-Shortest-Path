...
https://github.com/Xavier-MaYiMing/The-NSGA-II-for-the-multi-objective-shortest-path-problem
...
def transform_graph(graph):
    ...
    for node, edges in graph.items():
        ...
        for neighbor, (weight, probability) in edges.items():
            neighbor_index = node_map[neighbor]
            transformed_edges[neighbor_index] = [weight, probability]

        transformed_graph[node_index] = transformed_edges

    return transformed_graph, node_map

def map_nodes_to_letters(path, node_map):
    inverse_node_map = {v: k for k, v in node_map.items()}
    mapped_paths = []

    for p in path:
        mapped_path = {'path': [inverse_node_map[node] for node in p['path']], 'objective': p['objective']}
...
def write_csv(output_filepath, mapped_paths):
    file = open(output_filepath,"w")
    for p in mapped_paths:
        file.write(f"{p['objective'][0]},{p['objective'][1]}\n")

def main_nsga(source_node, destination_node, graph_filepath):
    graph = build_graph(graph_filepath)
    transformed_graph, node_map = transform_graph(graph)
    source_node = get_value_from_key(node_map, source_node)
    destination_node  = get_value_from_key(node_map, destination_node )
    paths = main(transformed_graph, source_node, destination_node)
    mapped_paths = map_nodes_to_letters(paths, node_map)
    return mapped_paths
